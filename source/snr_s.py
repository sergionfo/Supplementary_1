...
OUTPUT_CSV = "G:/AI/Data/snr_results_s.csv"
...
# -----------------------------
def compute_snr(trace, p_time, s_time):
    """
    ...
    if noise_end > p_time:
        ...
        noise_end = p_time - 0.5
    signal_start = s_time - 0.5
    signal_end = s_time + POST_WINDOW
...
for station, group in df.groupby(STATION_COL):
    ...
    for _, row in group.iterrows():
        try:
            ...
            if not (str(row[S_TIME_COL]).strip() == "" or  pd.isna(row[S_TIME_COL])):
                if (str(row[P_TIME_COL]).strip() == "" or  pd.isna(row[P_TIME_COL])):
                    p_time = UTCDateTime(row[S_TIME_COL])
                else:
    ...
    #                tr = tr[x]
                    # Compute SNR
                    snr = snr + compute_snr(tr[x], p_time, s_time)
                snr = snr/3
