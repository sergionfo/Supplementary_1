...
for evt in catalog:
    ...
    for i in range(len(outputs.picks)):
        if outputs.picks[i].phase == "P":
            staCode = outputs.picks[i].trace_id.split(".")[1]
            if staCode in catalog[evt].autoPick_P:
                if catalog[evt].manPick_P[staCode].time != "":
                    newt = UTCDateTime(outputs.picks[i].peak_time) - UTCDateTime(catalog[evt].manPick_P[staCode].time)
                    if catalog[evt].autoPick_P[staCode].time == "" and abs(newt) < t_dif:
                        catalog[evt].autoPick_P[staCode] = Picks(phase="P", time=outputs.picks[i].peak_time,fpos=0)
                    else:
                        ...
                        if catalog[evt].autoPick_P[staCode].time != "":
                            ...
                            if abs(newt) < abs(oldt):
                                catalog[evt].autoPick_P[staCode].time = time=outputs.picks[i].peak_time
                else:
                    catalog[evt].autoPick_P[staCode].falsePos = catalog[evt].autoPick_P[staCode].falsePos + 1
                    catalog[evt].autoPick_P[staCode].time = time=outputs.picks[i].peak_time

        if outputs.picks[i].phase == "S":
            staCode = outputs.picks[i].trace_id.split(".")[1]
            if staCode in catalog[evt].autoPick_S:
                if catalog[evt].manPick_S[staCode].time != "":
                    newt = UTCDateTime(outputs.picks[i].peak_time) - UTCDateTime(catalog[evt].manPick_S[staCode].time)
                    if catalog[evt].autoPick_S[staCode].time == "" and abs(newt) < t_dif:
                        catalog[evt].autoPick_S[staCode] = Picks(phase="S", time=outputs.picks[i].peak_time,fpos=0)
                    else:
                        ...
                        if catalog[evt].autoPick_S[staCode].time != "":
                            ...
                            if abs(newt) < abs(oldt):
                                catalog[evt].autoPick_S[staCode].time = outputs.picks[i].peak_time
                else:
                        catalog[evt].autoPick_S[staCode].falsePos = catalog[evt].autoPick_S[staCode].falsePos + 1
                        catalog[evt].autoPick_S[staCode].time = outputs.picks[i].peak_time
...
#---------------------------------------------------------------------------------------------------------
print("--- Write to CSV")
with open("./Compara/Todas_B/compare.csv", "w", newline="\n") as csvfile:
